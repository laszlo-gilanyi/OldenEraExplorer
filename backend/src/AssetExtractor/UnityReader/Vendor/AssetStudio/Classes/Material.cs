...
namespace AssetStudio
{
    ...
    internal sealed class Material : NamedObject
    {
        ...
        // MODIFIED FOR UnityAssetReader: keep ValidKeywords/InvalidKeywords +
        // CustomRenderQueue as fields. Upstream AssetStudioMod parsed them into
        // local variables and discarded; OEE's MaterialExtractor needs them for
        // shader-keyword-based emission detection (HEX_EMISSION_MAP_ENABLED) and
        // alpha-mode derivation from render queue.
        public string[] m_ValidKeywords = Array.Empty<string>();
        public string[] m_InvalidKeywords = Array.Empty<string>();
        public string m_LegacyShaderKeywords = string.Empty;
        ...
        public Material(ObjectReader reader, byte[] type, JsonSerializerOptions jsonOptions) : base(reader)
        {
            ...
            m_ValidKeywords = parsedMaterial.m_ValidKeywords ?? Array.Empty<string>();
            m_InvalidKeywords = parsedMaterial.m_InvalidKeywords ?? Array.Empty<string>();
            m_LegacyShaderKeywords = parsedMaterial.m_LegacyShaderKeywords ?? string.Empty;
        ...
        public Material(ObjectReader reader) : base(reader)
        {
            ...
            {
                // MODIFIED FOR UnityAssetReader: capture into fields instead of locals.
                m_ValidKeywords = reader.ReadStringArray();
                m_InvalidKeywords = reader.ReadStringArray();
            }
            else if (version >= 5) //5.0 - 2021.2.17
            {
                // MODIFIED FOR UnityAssetReader: capture into field instead of local.
                m_LegacyShaderKeywords = reader.ReadAlignedString();
            }
